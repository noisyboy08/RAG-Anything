...
try:
    ...
    def get_env_value(key, default=None, value_type=str):
        value = os.getenv(key)
        if value is None:
            return default
...
class RAGAnythingConfig:
    ...
    # ---
    enable_multimodal_embedding: bool = field(
        default=get_env_value("ENABLE_MULTIMODAL_EMBEDDING", False, bool)
    )
    """Enable true multimodal CLIP-based embeddings for images."""

    multimodal_embedding_model: str = field(
        default=get_env_value("MULTIMODAL_EMBEDDING_MODEL", "jinaai/jina-clip-v1", str)
    )
    """The model repository to use for true multimodal vectors."""

    enable_cross_encoder: bool = field(
        default=get_env_value("ENABLE_CROSS_ENCODER", False, bool)
    )
    """Enable cross-encoder reranking after LightRAG base retrieval."""

    cross_encoder_model: str = field(
        default=get_env_value("CROSS_ENCODER_MODEL", "cross-encoder/ms-marco-MiniLM-L-6-v2", str)
    )
    """The cross-encoder model to use for precision reranking."""
