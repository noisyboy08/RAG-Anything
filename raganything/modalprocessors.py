...
try:
    ...
    class EntityInfo(BaseModel):
        entity_name: str = Field(..., description="Unique entity name, normalized")
        entity_type: str = Field(..., description="Type of the entity")
        summary: str = Field(..., description="Summary of the entity")

    class ModalExtraction(BaseModel):
        detailed_description: str = Field(..., description="Detailed content description")
...
class BaseModalProcessor:
    ...
    def _robust_json_parse(self, response: str) -> dict:
        ...
        # Strategy 0: Pydantic Structured Output Validation
        if PYDANTIC_AVAILABLE:
            from pydantic import ValidationError
            json_candidates = self._extract_all_json_candidates(response)
            candidates_to_try = json_candidates if json_candidates else [response]
            
            for candidate in candidates_to_try:
                try:
                    parsed = ModalExtraction.model_validate_json(candidate)
