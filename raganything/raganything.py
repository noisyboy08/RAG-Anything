    ...
    async def _ensure_lightrag_initialized(self):
        ...
        try:
            ...
            if self.config.enable_cross_encoder:
                self.logger.info(f"Enabling Cross-Encoder: {self.config.cross_encoder_model}")
                try:
                    from sentence_transformers import CrossEncoder
                    encoder = CrossEncoder(self.config.cross_encoder_model)
                    
                    async def cross_encoder_rerank(query: str, texts: list[str]) -> list[float]:
                        if not texts:
                            return []
                    ...
                    lightrag_params["rerank_model_func"] = cross_encoder_rerank
                except ImportError:
                    self.logger.warning("sentence_transformers not installed. Cross-encoder disabled.")

            if self.config.enable_multimodal_embedding:
                self.logger.info(f"Enabling Multimodal Embeddings: {self.config.multimodal_embedding_model}")
                try:
                    ...
                    clip_model = SentenceTransformer(self.config.multimodal_embedding_model, trust_remote_code=True)
                    
                    async def multimodal_embed(inputs: list[str]) -> np.ndarray:
                        processed_inputs = []
                        for item in inputs:
                            # Heuristic: if it's base64 image or local file path
                            if item.startswith("data:image/") or ";base64," in item:
                                try:
                                    b64_data = item.split("base64,")[-1]
                                    img_data = base64.b64decode(b64_data)
                                    processed_inputs.append(Image.open(io.BytesIO(img_data)))
                                    continue
                                except Exception:
                                    pass
                            elif item.endswith((".png", ".jpg", ".jpeg")) and os.path.exists(item):
                                try:
                                    processed_inputs.append(Image.open(item))
                                    continue
                                except Exception:
                                    pass
                            processed_inputs.append(item)
                        
                        vectors = await asyncio.to_thread(clip_model.encode, processed_inputs)
                        return vectors
                        
                    # We inject our custom multimodal embedding wrapper over the default text embedding
                    lightrag_params["embedding_func"] = multimodal_embed
                except ImportError:
                    self.logger.warning("sentence_transformers or PIL not installed. Multimodal embeddings disabled.")
