...
class IndexManifest:
    ...
    def is_dirty(self, filepath: Path) -> bool:
        filepath = Path(filepath).resolve()
        if str(filepath) not in self.db:
            return True
        try:
            current_hash = self.get_file_hash(filepath)
            return self.db[str(filepath)]["hash"] != current_hash
        except Exception:
            return True
    ...
    def update(self, filepath: Path):
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"Failed to update manifest for {filepath}: {e}")
...
class BatchParser:
    ...
    def process_batch(
        ...
        **kwargs,
        ...
        manifest = IndexManifest(output_path / "index_manifest.json")
        ...
        for fp in supported_files:
            if manifest.is_dirty(Path(fp)):
                files_to_process.append(fp)
            else:
                self.logger.info(f"Skipping unmodified file: {fp}")
                successful_files.append(fp)
                
        self.logger.info(f"{len(files_to_process)} files need processing after delta check.")

        # Create progress bar if requested
        pbar = None
        if self.show_progress and files_to_process:
            pbar = tqdm(
            ...
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(
                    ...
                    if success:
                        ...
                        manifest.update(Path(file_path))
                    else:
    ...
    async def process_batch_async(
        ...
        **kwargs,
        ...
        async def process_with_semaphore(fp: str):
            async with semaphore:
                try:
                    # Run the synchronous parser in a background thread
                    success, processed_fp, err = await asyncio.to_thread(
                        self.process_single_file, fp, output_dir, parse_method, **kwargs
                    )
                    return success, processed_fp, err
                except Exception as e:
        ...
        tasks = [asyncio.create_task(process_with_semaphore(fp)) for fp in files_to_process]
